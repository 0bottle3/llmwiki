...
**전제**: 사내망(ALB `internal`) + 회사 IP 화이트리스트(SG) + 호스트네임 등록으로
이미 다층 방어가 걸려있다 (`14-network-access.md`, `15-zero-touch-onboarding.md`).
별도 IdP/토큰은 이 위협 모델에선 **과설계 + 마찰**.

**옵션**
- A. ~~Cognito + ALB 통합~~ — 인증 서버 추가 운영 + 팀원 로그인 셋업. 과설계
- B. ~~정적 토큰 + Secrets Manager~~ — 네트워크 게이트가 있으면 사실상 중복
- C. mTLS + 사내 PKI — MDM 있을 때 *강화용*으로만
- **E. 네트워크 게이트 + 호스트네임 식별 (zero-touch)** — 인증 서버 없음

**권장**: **E**
- 접속 자체가 회사 IP/VPN에서만 가능 (ALB internal + SG → 인터넷에 안 보임)
- 작성자 식별은 호스트네임 → ConfigMap 매핑 (`15-zero-touch-onboarding.md`)
- 팀원 셋업 0 (AWS 자격증명/토큰/로그인 전부 없음)

**한계 (감수)**: 회사망 안에 들어온 사람은 신뢰 → 사내 호스트네임 위장은 못 막음.

**강화가 필요해지면** (`15` 강화 경로):
- 내부자 위장 우려 ↑ → Level 2 (머신 ID) → Level 3 (mTLS/MDM)
- 외부 협력사 합류 → 그때 토큰 또는 OIDC 재검토
...
| D1. 벡터 DB | | | | |
| D2. 임베딩 모델 | | | | |
| D3. 인증 방식 | E (네트워크 게이트 + 호스트네임, Cognito/토큰 미채택) | | | 사내 전용 위협 모델 |
| D4. Vault 동기화 | | | | |
| D5. 가공 LLM | | | | |
| D6. 가공 실시간성 | | | | |
| D7. 다이제스트 채널 | | | | |
