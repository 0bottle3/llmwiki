...
> **⚠️ 그러나 — transcript는 "전 팀원 자동 공개"가 아니다 (C3).**
> 이 문서의 무손실 캡처는 *데이터를 잃지 않는 것*이 목표지, *전원에게 검색
> 공개*가 목표가 아니다. 아래 "프라이버시 정책"을 반드시 함께 적용한다.

---

## transcript 프라이버시 정책 (C3 — 가장 중요)

AI 대화 transcript를 통째로 빨아들여 전 팀원이 검색하게 만들면 세 가지가 터진다:

1. **시크릿/PII 유출**: 디버깅 중 AI에 붙여넣은 prod 로그·DB 덤프·고객 정보·
   실제 토큰이 vault로. regex 마스킹(`07-§6`)은 불완전 → 코드 문서처럼 새어나감.
2. **심리적 안전감 붕괴**: "alice가 어제 뭘 헤맸는지"를 bob이 전부 검색 가능하면
   사람들이 AI를 솔직하게 못 쓴다 → 시스템 가치 자체가 무너짐.
3. **위장 오염**: 호스트네임 위장(낮은 위협이라 감수)이 일어나면 남의 사적 대화가
   본인 것으로 섞임.
...
| 단계 | 기본값 |
|------|--------|
| 캡처(로컬 WAL) | ON (무손실이 목적) — **단 WAL write 전 마스킹**(M4) |
| S3 업로드 | ON, 단 `raw/conversations/{user}/` 는 **본인 전용 prefix** |
| 공용 검색 인덱스 | **OFF (기본)** — 본인만 검색 가능 |
| 팀 공유 | 명시적 opt-in한 세션/메시지만 (`share: team` 또는 `:vault:` 표식) |

- 12번이 슬랙엔 "자동 수집 X, 명시 등록만" 원칙을 적용했다. **더 민감한
  transcript에 같은 원칙을 적용**하는 것이 일관성이다.
- 가공 시 `owner`와 `sensitivity`를 부여: 개인 transcript는 `sensitivity: private`,
  공유 표식이 있으면 `internal`로 승격.
- search-api는 `sensitivity: private` 문서를 **요청자 본인에게만** 반환
  (호스트네임 식별로 owner 매칭). 이 필터는 Phase 1부터 필수(`07-§3`, M1).

### WAL 단계 마스킹 (M4)

WAL은 로컬 평문 JSONL이다. FileVault에만 의존하지 말고, **데몬이 WAL에 append
하기 *전*에** `07-§6`의 마스킹을 1차 적용한다. raw 보존이 필요한 경우에도
시크릿만은 write 전에 제거 — 평문 디스크에 토큰이 남지 않도록.
...
- **WAL write 전 시크릿 마스킹**(M4) — FileVault에만 의존하지 않음
- ingest API 인증: 없음 (회사 IP 게이트 + 호스트네임 식별, `14`/`15`)
- 멱등키에 user(서버가 호스트네임으로 결정) 포함 → 다른 사람 세션 덮어쓰기 불가
- transcript는 **기본 비공개**(본인 prefix), 공유 표식만 공용 인덱스 (위 프라이버시 정책)
